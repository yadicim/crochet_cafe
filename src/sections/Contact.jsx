...
export const Contact = () => {
    ...
    // 3. İletişim bilgileri etiketleri JSON'dan geliyor
    const contactInfo = [
        {
            icon: Mail,
            label: t('contact.info.label_1'),
            value: "yadigar.benli@gmail.com",
            href: "mailto:yadigar.benli@gmail.com",
        },
        {
            icon: Phone,
            label: t('contact.info.label_2'),
            value: "+358 46 593 31 54",
            href: "tel:+358465933154",
        },
        {
            icon: MapPin,
            label: t('contact.info.label_3'),
            value: "Oulu, Finland",
            href: "#"
        },
    ];
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setSubmitStatus({
                type: "success",
                message: t('contact.success_msg'), // 4. Başarı mesajı JSON'dan
            });
            setFormData({ name: "", email: "", message: "" });
        } catch (error) {
            setSubmitStatus({
                type: "error",
                message: t('contact.error_msg'), // 5. Hata mesajı JSON'dan
            });
        } finally {
            setIsLoading(false);
        }
    ...
    return (
        <section id="contact" className="py-32 relative overflow-hidden">
            ...
            <div className="absolute top-0 left-1/4 w-96 h-96 bg-(--color-primary)/5 rounded-full blur-3xl"></div>
            ...
            <div className="container mx-auto px-4 md:px-6 relative z-10">
                ...
                <div className="text-center mx-auto max-w-3xl mb-16">
                    ...
                    <h2 className="text-(--color-opposite) text-3xl md:text-5xl font-bold mt-4 mb-6 animate-fade-in animation-delay-100">
                        {t('contact.header_2')}{" "} 
                        <span className="font-serif italic font-normal text-white"> {t('contact.header_3')}</span>
                    </h2>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 max-w-5xl mx-auto">
                    {/* CONTACT FORM */}
                    <div className="glass p-6 md:p-8 rounded-3xl border border-(--color-primary)/30 animate-fade-in animation-delay-300">
                        <form className="space-y-6" onSubmit={handleSubmit}>
                            <div>
                                <label htmlFor="name" className="block text-sm font-medium mb-2">{t('contact.name')}</label>
                                <input id="name" type="text" required placeholder={t('contact.y_name')}
                                    value={formData.name}
                            ...
                            <div>
                                ...
                                <input id="email" type="email" required placeholder="mail@example.com"
                            ...
                            <div>
                                ...
                                <textarea id="message" rows={4} required placeholder={t('contact.y_name')} // İstersen JSON'a placeholder_msg ekleyebilirsin
                                    value={formData.message}
                                    onChange={(e) => setFormData({ ...formData, message: e.target.value })}
                                    className="w-full px-4 py-3 bg-(--color-surface) rounded-xl border border-(--color-border) focus:border-(--color-opposite) outline-none transition-all" />
                            </div>

                            <Button className="w-full text-black flex items-center justify-center gap-2" type="submit" disabled={isLoading}>
                    ...
                    <div className="space-y-6 animate-fade-in animation-delay-400">
                        <div className="glass rounded-3xl p-6 md:p-8">
                            ...
                            <div className="space-y-4">
                                {contactInfo.map((item, i) => (
                                    <a key={i} href={item.href} className="flex items-center gap-4 p-4 rounded-xl hover:bg-(--color-surface) transition-colors group">
                                        ...
                                        <div>
                                            <div className={isLight ? "text-slate-400" : "text-(--color-muted-foreground)"}>
                                                {item.label}
                                            </div>
                        ...
                        <div className="glass rounded-3xl p-6 md:p-8 border border-(--color-opposite)/30">
                            ...
                            <p className={isLight ? "text-slate-400" : "text-(--color-muted-foreground)"}>
                                {t('contact.card_p')}
                            </p>
                        </div>
                    </div>
