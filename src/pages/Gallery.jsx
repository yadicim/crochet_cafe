...
// --- 1. KART BİLEŞENİ (Dışarıda tanımlanması daha performanslıdır) ---
const ProjectCard = ({ item, translation, idx, isLight, phoneNumber, t }) => {
    // Her kartın kendi hata durumu olmalı
    const [videoError, setVideoError] = useState(false);

    return (
        <div className="group glass rounded-2xl overflow-hidden animate-fade-in hover:translate-y-2 transition-all duration-500 border border-white/5 flex flex-col" 
            ...
            <div className="relative aspect-4/5 overflow-hidden group/image-box bg-zinc-900">
                ...
                {item.video && !videoError ? (
                    <video 
                        ...
                        onError={() => setVideoError(true)} // Hata olursa resme döner
                    />
                ) : item.slides ? (
                    /* SLIDE GÖSTERİMİ */
                    ...
                    /* VARSAYILAN RESİM (Veya video hata verdiğinde) */
                    <img 
                        src={item.image} 
            ...
            <div className="p-6 space-y-4 text-(--color-opposite) grow">
                <div>
                    <span className="text-[10px] font-bold uppercase tracking-widest text-(--color-primary) mb-1 block">
                        {t(`projects.categories.${item.category}`, item.category)}
                    </span>
                ...
                <div className="flex flex-wrap gap-2 pt-2">
                    {item.tags?.map((tag) => (
                        <span key={tag} className="px-3 py-1 rounded-full bg-white/5 text-[11px] font-bold border border-white/10 text-(--color-highlight) uppercase tracking-tighter">
                            #{tag}
                        </span>
                    ))}
                </div>
            ...
            <div className="p-6 pt-0 mt-auto border-t border-white/5 flex items-center justify-between bg-white/5">
                ...
                {item.status !== 'sold_out' && (
                    <a href={`https://wa.me/${phoneNumber}?text=${encodeURIComponent(t('projects.wa_message', { item: translation.title }))}`} 
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                                {catItems.map((item, idx) => (
                                    <ProjectCard 
                                        key={item.id} 
                                        item={item} 
                                        translation={t(`projects.items.${item.id}`, { returnObjects: true })} 
                                        idx={idx} 
                                        isLight={isLight} 
